...
def list_purchases():
    ...
    pagination = Purchase.query.order_by(Purchase.purchase_date.desc()).paginate(page=page, per_page=15, error_out=False)
    return render_template('purchases/list.html', purchases=pagination.items, pagination=pagination)
...
def bulk_action():
    ...
    if action == 'delete':
        try:
            ...
            db.session.commit()
            flash(f'{len(purchases)} purchases deleted and stock adjusted.', 'success')
        except Exception as e:
            db.session.rollback()
